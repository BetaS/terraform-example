  ...
  container_definitions = jsonencode([
    {
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          "awslogs-group"         = aws_cloudwatch_log_group.web-server.name
...
resource "aws_security_group_rule" "lb_to_web_server_egress" {
...
resource "aws_iam_role" "task_role_web" {
  name = "ecsTaskRole-${var.name}-web"
